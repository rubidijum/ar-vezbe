...
TermPtr FunctionT(std::string f, const std::vector<TermPtr>& args)
{ return std::make_shared<Term>(FunctionTerm{f, args}); }
...
TermPtr substitute(TermPtr term, const std::string &v, TermPtr t) {
    ...
    case Term::Function:
        ...
        return FunctionT(term->function.symbol, subterms);
    }
}
...
FormulaPtr substitute(FormulaPtr formula, const std::string &v, TermPtr t) {
    ...
    case Formula::Atom: {
        ...
        for(const auto& subterm : formula->atom.args)
            subterms.push_back(substitute(subterm, v, t));
        return Atom(formula->atom.symbol, subterms);
    }
    case Formula::Not:
        return Not(substitute(formula->negation.subformula, v, t));
    case Formula::Binary:
        return Binary(formula->binary.type,
    ...
    case Formula::Quantifier: {
        ...
        if(containsVariable(t, formula->quantifier.var)) {
            ...
            TermPtr varTerm = VariableT(uniqueVar);
...
FormulaPtr simplifyAnd(FormulaPtr formula) {
    ...
    if(simplifiedR->type == Formula::Type::True)
        return simplifiedL;
    return And(simplifiedL, simplifiedR);
}
...
FormulaPtr simplifyOr(FormulaPtr formula) {
    ...
    if(simplifiedR->type == Formula::Type::False)
        return simplifiedL;
    return Or(simplifiedL, simplifiedR);
}
...
FormulaPtr simplifyImpl(FormulaPtr formula) {
    ...
    if(simplifiedL->type == Formula::Type::True)
        return simplifiedR;
    if(simplifiedR->type == Formula::Type::False)
        return simplify(Not(simplifiedL));
    return Impl(simplifiedL, simplifiedR);
}
...
FormulaPtr simplifyEql(FormulaPtr formula) {
    ...
    if(simplifiedR->type == Formula::Type::True)
        return simplifiedL;
    if(simplifiedL->type == Formula::Type::False)
        return simplify(Not(simplifiedR));
    if(simplifiedR->type == Formula::Type::False)
        return simplify(Not(simplifiedL));
    return Eql(simplifiedL, simplifiedR);
}
...
FormulaPtr skolem(FormulaPtr formula, Signature& s, std::set<std::string>& vars) {
    ...
    for(const auto& var : vars)
        args.push_back(VariableT(var));
    TermPtr uniqueTerm = FunctionT(uniqueFun, args);
